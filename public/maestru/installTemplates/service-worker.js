...
self.addEventListener('push', function (event) {
    ...
    if (typeof title === "string" && title.length > 2) {
        event.waitUntil(
            fetch(HOME + "get/push/receivePush/" + options.id).then(() => {
...
self.addEventListener('notificationclick', function (event) {
    ...
    event.waitUntil(
        fetch(HOME + "get/push/openPush/" + event.notification.tag).then(() => {
                    ...
                    if(response) {
                        ...
                        return fetch(e.request).then(networkResponse => {
                            if (networkResponse && networkResponse.status === 200 && networkResponse.type === 'basic') {
                                ...
                                if (["get/appFilesView", "get/currentFiles", "get/userCache", "get/appFilesViewUser", "get/entity/recoverySync"].indexOf(url) === -1 && !/get\/event\//.test(url))
                                    cache.put(url, networkResponse.clone());
