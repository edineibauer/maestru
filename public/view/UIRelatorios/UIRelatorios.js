...
$(function ($) {
    ...
    $("#app").off("click", ".btn-report-advanced").on("click", ".btn-report-advanced", function () {
        ...
        AJAX.post("report/saveFieldsGrid", {type: "report", entity: report.id, fields: report.fields});
        ...
        AJAX.post("report/create/grafico", {
            x: x,
            y: y,
        ...
        if(confirm("excluir gráfico?")) {
            AJAX.post("report/delete/grafico", {id: id}).then(() => {
                updateGraficos().then(() => {
                    readGraficosTable(identificador)
                })
            })
