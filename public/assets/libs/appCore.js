...
function setUpdateVersion() {
    return AJAX.post("config/updateVersion", {update: !0}).then(data => {
...
async function checkUpdate() {
    if (navigator.onLine && SERVICEWORKER) {
        let version = await AJAX.post("config/updateVersion");
...
function loadSyncNotSaved() {
    ...
    return AJAX.get('entity/recoverySync').then(sync => {
...
function clearCacheUser() {
    ...
    for (let entity in dicionarios) {
        clear.push(dbLocal.exeRead("sync_" + entity).then(d => {
            ...
            AJAX.post("entity/up/sync", {entity: entity, dados: d});
...
function updateGraficos() {
    return dbLocal.clear('__graficos').then(() => {
        return AJAX.get("config/graficos").then(r => {
...
function acceptInstallApp() {
    if (!localStorage.installAppAction) {
        ...
        deferredPrompt.userChoice.then(choiceResult => {
            ...
            if (localStorage.installAppAction)
                AJAX.post("config/appInstaled", {success: !0, ios: isIos()});
            else
                AJAX.post("config/appInstaled", {success: !1, ios: isIos()});
        });
    }
}
...
function closeInstallAppPrompt(onInstall) {
    ...
    AJAX.post("config/appInstaledPrompt", {success: typeof onInstall !== "undefined", ios: isIos()});
...
async function setUserData(field, value) {
     ...
     */
    if(navigator.onLine)
        return AJAX.post("config/setUserData", updates);
}
